...
menu_dataset  = MenuDataset(dataset_name + "_price.csv", dataset_name + "_vocab_v2.p", sampling="word2vec", encoding="utf-8", item_column_name="description", factor_column_name="category", price_column_name="price")
...
model = wp(menu_dataset.n_vocab, H, menu_dataset.n_vocab)
model.to(device)
...
for t in range(n_epochs):
    ...
    for i_batch, sample_batched in enumerate(train_loader):
        ...
        y = sample_batched["target_word"].squeeze().to(device)
    ...
    torch.save(model.state_dict(), "../models/word2vec_" + dataset_name + "_" + str(t))
