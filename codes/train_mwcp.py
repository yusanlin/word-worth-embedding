...
menu_dataset  = MenuDataset(dataset_name + "_price.csv", dataset_name + "_vocab_v2.p", sampling="mwcp", encoding="utf-8", item_column_name="description", factor_column_name="category", price_column_name="price")
...
model = mwcp(menu_dataset.n_vocab, menu_dataset.n_factors, H)
model.to(device)
...
for t in range(n_epochs):
    ...
    for i_batch, sample_batched in enumerate(train_loader):
        ...
        X_word = sample_batched["context_words"].to(device)
        X_factor = sample_batched["factor"].to(device)
        y_word = sample_batched["target_word"].squeeze().to(device)
        y_price = sample_batched["price"].to(device)

        # forward pass
        y_word_pred, y_price_pred = model(X_word, X_factor)
    ...
    torch.save(model.state_dict(), "../models/mwcp_"  + dataset_name + "_" + str(t))
